...
(Để vẽ sơ đồ chuyên nghiệp, bạn nên sử dụng công cụ Fritzing hoặc Tinkercad. Nguyên lý: Cảm biến TCRT đọc trạng thái binary (đen=0, trắng=1) để tính error; HC-SR04 sử dụng công thức distance = (duration * 0.034)/2 cm; PID tính toán: Output = KpError + Ki∫Error + KddError/dt.)*

Sơ đồ chi tiết: image
